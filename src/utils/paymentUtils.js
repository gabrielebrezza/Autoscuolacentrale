...
async function createPaypal(price, returnPath, custom = {}){
    const create_payment_json = {
        intent: "sale",
        payer: {
            payment_method: "paypal"
    ...
    return new Promise((resolve, reject) => {
        paypal.payment.create(create_payment_json, async (error, payment) => {
            if (error) {
                ...
                for (let i = 0; i < payment.links.length; i++) {
                    if (payment.links[i].rel === "approval_url") {
                        resolve({url: payment.links[i].href, paymentId: payment.id});
                    }
                }
            }
        });
...
async function retrivePayPal(username, payerId, paymentId, type){
    return new Promise((resolve, reject) => {
        paypal.payment.execute(paymentId, {payer_id: payerId}, async (error, payment) =>{
            if(error){
                reject(error);
                ...
                const paymentUrl = `https://paypal.com/activity/payment/${transactionId}`;
                const custom = JSON.parse(payment.transactions[0].custom);
                await addFattura(username, paymentUrl, type, payment.transactions[0].amount.total);
                resolve({ custom });
            }
        });
    });
...
async function createSatispay(price, lessonId, returnPath, custom = {}) {
    const payload = {
        flow: 'MATCH_CODE',
        amount_unit: price * 100,
        currency: 'EUR',
        callback_url: `${process.env.SERVER_URL}/satispay-callback?payment_id={uuid}`,
        redirect_url: `${process.env.SERVER_URL}${returnPath}?paymentMethod=satispay&lessonId=${lessonId}`,
        metadata: custom
    };
    ...
    try {
        ...
        return { url: response.data.redirect_url, paymentId: response.data.id };
    } catch (error) {
        console.error('Error creating payment:', error.response ? error.response.data : error.message);
    }
}
...
//DA RIATTIVARE LA FATTURAZIONE
async function checkCode(code, amount, userId, type) {
    const result = await CodesDB.findOneAndUpdate({ user: userId, code: code, amount: amount, active: true }, { usedAt: new Date(), active: false });
...
async function setLessonPaid(userId, lessonId){
    const lesson = await LessonsDB.findById(lessonId)

    const paymentStartTime = new Date(lesson.payment.paymentCreatedAt);
    if (lesson.payment.status === "completed") return { expired: true };
    const currentTime = new Date();
    const expirationTime = 30 * 60 * 1000;
  
    if (currentTime.getTime() - paymentStartTime.getTime() > expirationTime) return {expired: true};

    await LessonsDB.findByIdAndUpdate(lesson._id, { "student": userId, "payment.status": 'completed' });
    ...
    content = content.replace(/\(NOME\)/g, billingInfo[0].nome)
    .replace(/\(COGNOME\)/g, billingInfo[0].cognome)
    .replace(/\(DATA\)/g, lesson.day.toLocaleDateString('it-IT'))
    .replace(/\(DAORA\)/g, lesson.startTime)
    .replace(/\(AORA\)/g, lesson.endTime)
    .replace(/\(LINKPOSIZIONE\)/g, lesson.locationLink);
    try{
        const result = await sendEmail(email, subject, content);
        console.log(result);
    }catch(error){
        console.log(`errore durante l'invio dell'email per la conferma della prenotazione: ${error}`, );
    }
    return {expired: false};
}
...
async function setSpostaGuidaPaid(userId, custom) {
    const { oldLessonId, newLessonId } = custom;
    const oldLesson = await LessonsDB.findById(oldLessonId);
    const newLesson = await LessonsDB.findById(newLessonId);

    if (!oldLesson || !newLesson) return { status: 404, message: "Lezione non trovata" };

    if (newLesson.payment?.status === "completed") return { status: 409, message: "La lezione scelta è già stata prenotata" };

    const paymentStartTime = newLesson.payment?.paymentCreatedAt ? new Date(newLesson.payment.paymentCreatedAt) : null;
    const currentTime = new Date();
    const expirationTime = 30 * 60 * 1000;

    if (paymentStartTime && currentTime.getTime() - paymentStartTime.getTime() > expirationTime) return { status: 410, message: "Tempo per completare il pagamento scaduto" };
    ...
    await LessonsDB.findByIdAndUpdate(newLessonId, { "student": userId, "payment.status": "completed"});

    return { status: 200, message: "Spostamento completato con successo" };
}
